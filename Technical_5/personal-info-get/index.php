...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['submit_btn'])) {
    $fname   = htmlspecialchars($_GET['firstname']     ?? '');
    $mname   = htmlspecialchars($_GET['middlename']    ?? '');
    $lname   = htmlspecialchars($_GET['lastname']      ?? '');
    $b_year  = htmlspecialchars($_GET['birth_year']    ?? '');
    $b_month = htmlspecialchars($_GET['birth_month']   ?? '');
    $b_day   = htmlspecialchars($_GET['birth_day']     ?? '');
    $city    = htmlspecialchars($_GET['city']          ?? '');
    $country = htmlspecialchars($_GET['country']       ?? '');
    $local   = htmlspecialchars($_GET['local_address'] ?? '');
...
<head>
    ...
    <title>Personal Information Form (GET)</title>
...
<div class="card">
    ...
    <?php else: ?>
        <div class="badge-info">
            Congratulations!<br>
            You just used PHP Superglobal - $_GET.
        </div>
